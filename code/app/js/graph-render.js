export function renderGraph(nodes, links) {
  ...
  nodesMap.forEach(n => {
    ...
    if (typeof n.x !== "number" || isNaN(n.x)) n.x = W / 2;
  ...
  function update() {
    ...
    topRoots.forEach(collectVisible);
    ...
    nodesMerged
      .on("mouseover", (e, d) => {
        ...
        if (d.type.endsWith("Event")) {
          ...
          const exclude = new Set(["id", "type", "children", "collapsed", "x", "y", "vx", "vy", "index", "fx", "fy"]);
  ...
  function ticked() {
    linksMerged
      ...
      .attr("y1", d => (typeof d.source === "object" ? d.source.y : nodesMap.get(d.source)?.y || 0))
      .attr("x2", d => (typeof d.target === "object" ? d.target.x : nodesMap.get(d.target)?.x || 0))
      .attr("y2", d => (typeof d.target === "object" ? d.target.y : nodesMap.get(d.target)?.y || 0));
