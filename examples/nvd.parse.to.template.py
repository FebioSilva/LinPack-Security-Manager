...
def write_to_file(cve_list, output_file):
    with open(output_file, 'w', encoding='utf-8') as file:
        for cve in cve_list:
            ...
            file.write(f"  - CVSS Version: {cve['severity'].get('cvssVersion')}\n")
            file.write(f"  - Base Score: {cve['severity'].get('baseScore')}\n")
            file.write(f"  - Base Severity: {cve['severity'].get('baseSeverity')}\n")
            file.write(f"  - CVSS Vector: {cve['severity'].get('cvssCode')}\n")
            
            file.write("CPEs:\n")
            if cve['cpe']:
                for cpe in cve['cpe']:
                    file.write(f"  - Vendor: {cpe.get('vendor')}, Product: {cpe.get('product')}, Version: {cpe.get('version')}\n")
            else:
