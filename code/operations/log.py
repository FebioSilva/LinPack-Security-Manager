...
def process_dpkg_log(input_file):
    ...
    for log in parser.parsed_logs:
        ...
        if log["type"] == "action":
            ask_for_package_query = logAuxiliary.ask_for_package_to_sparql(log, graph_uri)
            pkg_exists = dbOperations.ask_for_package(ask_for_package_query)["boolean"]

            if pkg_exists and log["action"] in ["install", "remove", "upgrade", "trigproc", "purge"]:
                delete_package_query = logAuxiliary.delete_package_to_sparql(log, graph_uri)
        ...
        log_in_sparql = logToRDF.dpkg_log_to_sparql(log, matched_product, graph_uri)
        print(log_in_sparql)
